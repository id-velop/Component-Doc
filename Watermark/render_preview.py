# -*- coding: utf-8 -*-
"""生成 Watermark-preview-baked.html。依赖: pip install markdown

轻量预览：同目录 Watermark-preview.html（fetch Watermark.md，需 HTTP 服务）。
"""
import re
import sys
...
try:
    ...
    print("需要安装: pip install markdown", file=sys.stderr)
    sys.exit(1)

DIR = Path(__file__).resolve().parent
MD = DIR / "Watermark.md"
OUT = DIR / "Watermark-preview-baked.html"
...
<style>
  ...
  .demo-mount {{ margin: 10px 0; padding: 12px 14px; border: 1px solid #e8e8e8; border-radius: 8px; background: #fff; min-height: 48px; max-width: 720px; position: relative; overflow: visible; transform: translate(0); }}
